...
export default function DayContainer({ day, allDays }: Props) {
  const response = useSWR('/api/days', {
    initialData: allDays,
    refreshInterval: 5000
  });

  const updatedDays = response.data || [];
  const updatedDay = updatedDays.find((s: Day) => s.slug === day.slug) || day;
  ...
  return (
    <div className={styles.container}>
        ...
        {loginStatus === 'loggedIn' ? (
          <div className={cn(styles.stream, styleUtils.appear, styleUtils['appear-first'])}>
            <iframe
              ...
              allowFullScreen
              frameBorder="0"
              src={`${updatedDay.stream}?autoplay=1&mute=1`}
              title={updatedDay.name}
              width="100%"
            />
      ...
      <ScheduleSidebar allDays={updatedDays} />
    </div>
  );
}
